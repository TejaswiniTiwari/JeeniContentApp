...
import static com.jeeni.content.classic.utils.Utils.DIRECTORY_MY_FILES;

public class MyFilesActivity extends AppCompatActivity implements UnitDetailsCallBacks, DownloadCallBacks,DownloadProgressCallBacks {
...
//        databaseAdapter = new DatabaseAdapter(context);
        ...
        textViewSubTitle.setText("My Files");
    }
    ...
    private void showCourses() {
        ...
        unitContents.clear();
        try {
            ArrayList<String> myFiles=Utils.getAllMyFiles(MyFilesActivity.this);
            for(String s:myFiles){
                UnitContent unitContent=new UnitContent("","","","",s,s.toLowerCase().contains("mp4"),false,true);
...
//            unitContents = databaseAdapter.getLatestUnitContent();
    ...
    private void showList(boolean chechDownloadStatus) {
        ...
        recyclerView.addItemDecoration(new MyFilesActivity.SpaceItems(10));
    ...
    private void checkDownloadStatus() {
        for (UnitContent unitContent : unitContents) {
            try {
                if (Utils.isFileExist(MyFilesActivity.this, DIRECTORY_MY_FILES+ File.separator+unitContent.getFileName())) {
...
//            (new DatabaseAdapter(context)).updateLastUsedFiles(unitContent.getFileName(),unitContent.getCourseId(),unitContent.getSubjectId(),unitContent.getUnitId(),System.currentTimeMillis());
            ...
            if (unitContent.isVideo()) {
                new ContentViewer().viewVideo(unitContent,unitContent.getFileName(), context);
            } else {
                new ContentViewer().viewNonEncryptedPdf(DIRECTORY_MY_FILES+ File.separator+unitContent.getFileName(), context);
            }
        }
